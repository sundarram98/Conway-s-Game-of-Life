...
def test_neighbors(board):
    ...
    for block in board.block_list.values():
        ...
        ncoords = [block.get_coords() for neighbor in neighbors]
...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        self.canvas.setBackground('grey')
    ...
    def draw_gridline(self, startp, endp):
        ...
        '''
        line = Line(Point(startp.x * BLOCK_SIZE, startp.y * BLOCK_SIZE), Point(endp.x * BLOCK_SIZE, endp.y * BLOCK_SIZE))
        line.draw(self.canvas)

        line = Line(Point(startp.x * BLOCK_SIZE - 1, startp.y * BLOCK_SIZE - 1), Point(endp.x * BLOCK_SIZE - 1, endp.y * BLOCK_SIZE - 1))
        line.draw(self.canvas)
    ...
    def seed(self, block_coords):
        ...
        for pt in block_coords :
            self.block_list[pt].set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        # Initializing the neighbors list
        neighbours = []
            ...
            for j in range(-1,2) :
                if i == 0 and j == 0 :
                    ...
                    if x+i >= 0 and x+i < self.width and y+j >= 0 and y+j < self.height :
                        neighbours.append(self.block_list[(x+i,y+j)])


        return neighbours

    def number_live_blocks( self, block) :
        # This function computes the number of live blocks around a block

        #Finding neighbors
        block_list = self.get_block_neighbors(block)

        # Store the number of live blocks in count
        count = 0

        # If a block is live, we increment count by 1
    ...
    def simulate(self):
        ...
        for block in self.block_list.values():
            #Counting number of live neighbors of the block
            count = self.number_live_blocks(block)
...
if __name__ == '__main__':
    ...
    ## Make sure that the board __init__ method works
    # board.random_seed(.35)

    ## Make sure board.seed works. Comment random_seed above and uncomment
    ...
    ## Test that neighbors work by commenting the above and uncommenting
